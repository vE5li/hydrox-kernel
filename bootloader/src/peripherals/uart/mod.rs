...
use peripherals::gpio::{ Pin, Pull, Function, set_function, set_pull, idle };
...
pub fn initialize() {
    ...
    unsafe {
        ...
        write_volatile(&mut (*registers).aux_enables, 0x1);
        write_volatile(&mut (*registers).aux_mu_ier_reg, 0);
        write_volatile(&mut (*registers).aux_mu_cntl_reg, 0);
        write_volatile(&mut (*registers).aux_mu_lcr_reg, 0x3);
        ...
        write_volatile(&mut (*registers).aux_mu_cntl_reg, 0x3);
    }
