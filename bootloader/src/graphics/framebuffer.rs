...
impl Framebuffer {
    ...
    pub fn draw_character(&mut self, x_position: usize, y_position: usize, character: char, color: u32) {
        ...
        for y in 0..FONT_HEIGHT {
            for x in 0..FONT_WIDTH {
                ...
                if glyph[y] & mask != 0 {
                    self.draw_pixel(x_position + x, y_position + y, color);
                }
            }
        }
    }
...
pub fn initialize() -> Framebuffer {
    ...
    log_line!("[ graphics ] monitor width: {}", width); // remove :x
    log_line!("[ graphics ] monitor height: {}", height); // remove :x
    ...
    let index = message.tag_index(MailboxTag::SetFramebufferDepth).expect("invalid framebuffer depth response");
    let depth = message.buffer()[index + 3] as usize;

    let index = message.tag_index(MailboxTag::SetAlphaMode).expect("invalid framebuffer alpha mode response");
    let alpha_mode = message.buffer()[index + 3] as usize;

    let index = message.tag_index(MailboxTag::SetPixelOrder).expect("invalid framebuffer pixel order response");
    let pixel_order = message.buffer()[index + 3] as usize;

    let index = message.tag_index(MailboxTag::AllocateFramebuffer).expect("invalid allocate framebuffer response");
    ...
    log_line!("[ graphics ] framebuffer size: {}", size);
